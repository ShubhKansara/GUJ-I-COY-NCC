...
Route::group($default, function () {
    ...
    Route::group(['middleware' => ['boilerplateauth', 'ability:admin,backend_access']], function () {
        ...
        Route::get('/demo', ['as' => 'demo', 'uses' => config('boilerplate.menu.dashboard').'@demo']);
    });
    ...
    Route::group(['namespace' => '\App\Http\Controllers\Boilerplate'], function () {
        ...
        // Backend
        Route::group(['middleware' => ['boilerplateauth', 'ability:admin,backend_access']], function () {
            Route::post('chatbots/copy-chatbots/{id?}', ['as' => 'chatbots.copyChatbot', 'uses' => 'ChatbotsController@copyChatbot']);

            // Roles and users
            Route::resource('roles', 'Users\RolesController', ['except' => 'show']);
            Route::resource('users', 'Users\UsersController', ['except' => 'show']);

            Route::resource('institutes', 'InstituteController', ['except' => 'show']);
            Route::any('institutes/dt', ['as'=>'institutes.dt' ,'uses'=>'InstituteController@datatable']);
            Route::resource('candidate', 'CandidateController', ['except' => 'show']);
            Route::any('candidate/dt', ['as'=>'candidate.dt' ,'uses'=>'CandidateController@datatable']);
            Route::any('filter-candidate/dt', ['as' => 'filter-candidate.datatable', 'uses' => 'CandidateController@FilterCandidateDatatable']);

            Route::post('/download_candidate', ['as' => 'candidate-data-download', 'uses' => 'CandidateController@DownloadCandidateData']);

            Route::resource('newUsers', 'NewUserController', ['except' => 'show']);
