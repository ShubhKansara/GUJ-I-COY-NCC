...
@endisset
@if($maximize)
            <button type="button" title="Maximize" class="btn btn-tool" data-card-widget="maximize"><i class="fas fa-expand"></i></button>
@endif
@if($reduce)
            <button type="button" title="Collapse" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-{{ $collapsed ? 'plus' : 'minus' }}"></i></button>
@endif
@if($close)
