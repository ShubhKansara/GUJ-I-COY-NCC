...
<head>
    ...
    {{-- <link rel="shortcut icon" href="{{ config('boilerplate.theme.favicon') ?? mix('favicon_extrahours.svg', 'assets/vendor/boilerplate') }}"> --}}
    <link rel="shortcut icon" href="{{ asset('assets/vendor/boilerplate/images/vendor/favicon_new.png') }}">
@stack('plugin-css')
    ...
    <style type="text/css">
        .page-link,.page-link:hover{color: white;}
    </style>
@stack('css')
</head>
<body class="layout-fixed layout-navbar-fixed sidebar-mini{{ setting('darkmode', false) && config('boilerplate.theme.darkmode') ? ' dark-mode accent-light' : '' }}{{ setting('sidebar-collapsed', false) ? ' sidebar-collapse' : '' }}">
    <div class="wrapper">
        @include('boilerplate::components.spinner')
