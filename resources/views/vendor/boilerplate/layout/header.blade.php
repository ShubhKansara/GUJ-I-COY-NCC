<nav class="main-header navbar navbar-expand{{ config('boilerplate.theme.navbar.bg') === 'white' ? '' : ' navbar-'.config('boilerplate.theme.navbar.bg') }} navbar-{{ setting('darkmode', false) && config('boilerplate.theme.darkmode') ? 'dark' : config('boilerplate.theme.navbar.type') }} {{ config('boilerplate.theme.navbar.border') ? "" : "border-bottom-0" }}" data-type="{{ config('boilerplate.theme.navbar.type') }}">
    <div class="navbar-left d-flex">
        <ul class="nav navbar-nav">
            <li class="nav-item">
                <a class="nav-link sidebar-toggle px-2" title="Toggle Menu"  data-widget="pushmenu" href="#">
    ...
    @if(\Auth::user())
    <div class="navbar-center ml-auto d-flex text-center justify-content-center">
        <b>@if(empty(\Auth::user()->full_name)) {{\Auth::user()->first_name}} {{\Auth::user()->last_name}} @else {{ \Auth::user()->full_name }} @endif</b>&nbsp;{{ ' - ' . \Auth::user()->email }}
    </div>
    @endif
...
@endforeach
            ...
            <li class="nav-item">
                ...
                <button title="Log Out" type="submit" class="btn nav-link d-flex align-items-center logout px-2" data-question="{{ __('boilerplate::layout.logoutconfirm') }}">
